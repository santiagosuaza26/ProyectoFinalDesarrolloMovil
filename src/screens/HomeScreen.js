...
export function HomeScreen() {
    ...
    const fare = useMemo(() => {
        ...
        return calculateEstimatedFare(estimate.distanceKm, estimate.durationMinutes, selectedVehicle);
    }, [estimate, selectedVehicle]);
    async function handleDestinationSelected(nextDestination) {
        ...
        try {
            ...
            Alert.alert('Error', requestError instanceof Error
    ...
    return (<Screen scroll={false}>
      <View style={styles.container}>
        <MapView provider={PROVIDER_GOOGLE} style={styles.map} initialRegion={initialRegion} region={origin ? { ...initialRegion, ...origin } : initialRegion} showsUserLocation>
          {origin ? <Marker coordinate={origin} title={t('currentLocation')}/> : null}
          {destination ? (<Marker coordinate={destination} title={destination.address}/>) : null}
          {origin && destination ? (<MapViewDirections apikey={env.googleMapsApiKey} destination={destination} origin={origin} strokeColor="#2563eb" strokeWidth={4}/>) : null}
        </MapView>

        <View style={styles.panel}>
          <GooglePlacesAutocomplete debounce={300} enablePoweredByContainer={false} fetchDetails placeholder={t('destinationPlaceholder')} onPress={(data, details) => {
          ...
          <VehicleSelector value={selectedVehicle} onChange={category => dispatch(setSelectedVehicle(category))}/>
          ...
          <AppButton disabled={!destination || !estimate} loading={loading} onPress={handleRequestRide} title={t('requestRide')}/>
        </View>
      </View>
    </Screen>);
}
